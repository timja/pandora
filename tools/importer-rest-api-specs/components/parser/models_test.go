...
func TestParseModelSingleInheritingFromObjectWithNoFieldsDefined(t *testing.T) {
	result, err := ParseSwaggerFileForTesting(t, "model_inheriting_from_other_model_with_no_fields_defined.json")
	if err != nil {
		t.Fatalf("parsing: %+v", err)
	}
	if result == nil {
	...
	if *world.ResponseObject.ReferenceName != "Target" {
		t.Fatalf("expected the response object to be 'Target' but got %q", *world.ResponseObject.ReferenceName)
	}
	if world.ResourceIdName != nil {
		t.Fatalf("expected no ResourceId but got %q", *world.ResourceIdName)
	}
	...
	// whilst the response model references SecondObject, it's only inheriting from FirstObject, so it'll be switched out
	target, ok := hello.Models["Target"]
	if !ok {
		t.Fatalf("expected there to be a model called Target")
	}
	if len(target.Fields) != 1 {
		t.Fatalf("expected the model Target to have 1 field but got %d", len(target.Fields))
	}
	props, ok := target.Fields["props"]
	if !ok {
		t.Fatalf("expected the model Target to have a field named `Properties` but didn't get one")
	}

	if props.ObjectDefinition.Type != models.ObjectDefinitionReference {
		t.Fatalf("expected the second model TargetProperties to be a Reference")
	}

	if props.ObjectDefinition.ReferenceName == nil {
		t.Fatalf("expected the second model TargetProperties to have a reference name")
	}
}
